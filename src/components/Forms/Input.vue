<template>
    <div v-if='!isTextArea' class="input" :class="{error : error.length > 0}">
        <input type="text" @change="inputChanged" :placeholder="text">
        <div v-if="error.length > 0"><p class="error">{{error}}</p></div>
    </div>
    <div v-else class="input">
        <textarea @change="inputChanged" :placeholder="text" cols="30" rows="10"></textarea>
    </div>
</template>

<script>
export default {
    props: ['name', 'text', 'isTextArea', 'error'],
    emits: ['inputChanged'],
    setup(props, context) {
        function inputChanged(event) {
            /*console.log("Zmena : " + event.target.value);*/
            context.emit('inputChanged', {'name' : props.name, 'data': event.target.value});
        }
        return {inputChanged};
    },
}
